...
# QUADRANTE 2 - Classe de Recomendação 

class SistemaRecomendacaoMedica:
    def __init__(self, k=13):
        self.k = k                     # Melhor K já definido a partir dos testes
        self.modelo = None
...
# QUADRANTE 3 - Flask: Rotas e Inicialização do Modelo
...
def index():
    ...
    if request.method == 'POST':
        ...
        if selected:
            sintomas_lista = [s.strip() for s in selected.split(',') if s.strip()]
            try:
                pred = sistema.prever_por_nomes_sintomas(sintomas_lista)
                result = (
                    f"Especialista: {pred['especialista']} | "
                    f"Diagnóstico: {pred['doenca']} | "
                    f"Confiança: {pred['confianca']:.0%}"
                )
...
def inicializar():
    ...
    sistema = SistemaRecomendacaoMedica(k=13)  # melhor K definido a partir dos testes
...
# QUADRANTE 4 - Execução da Aplicação
